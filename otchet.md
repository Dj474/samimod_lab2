...
- R1 — число вылетевших рейсов за горизонт (**дискретный отклик**);
- R2 — долю рейсов, выполненных самолётами повышенной грузоподъёмности (**дискретный отклик**);
- R3 — среднее число контейнеров на складе (**непрерывный отклик**);
- R4 — среднее время ожидания контейнера до загрузки (**непрерывный отклик**);
- R5 — среднее число параллельно выполняемых загрузок (**непрерывный отклик**);
- R6 — суммарный вес вывезенного груза, т (**непрерывный отклик**).
...
```
прибыло тонн = вывезено тонн + остаток на складе + груз в самолётах в процессе загрузки
```

По 100 прогонам максимальное расхождение составило **5.7·10⁻⁹ т** (погрешность
округления с плавающей точкой) — баланс выполняется точно.
...
|---|---|---|---|---|---|
| **R1** число рейсов (дискр.) | 3293.03 | 28.77 | 3237 | 3356 | 5.72 |
| **R2** доля рейсов повыш. ГП (дискр.) | 0.2440 | 0.0064 | 0.229 | 0.257 | 0.0013 |
| **R3** ср. контейнеров на складе (непр.) | 22.58 | 3.99 | 16.62 | 39.53 | 0.79 |
| **R4** ср. время ожидания, ч (непр.) | 7.40 | 1.32 | 5.39 | 12.98 | 0.26 |
| **R5** ср. параллельных загрузок (непр.) | 0.45 | 0.004 | 0.439 | 0.460 | 0.0007 |
| **R6** вывезено груза, тыс. т (непр.) | 525.58 | 3.41 | 514.26 | 534.77 | 0.68 |
...
3. Верификация по повторным прогонам (100 прогонов) показала точное
   выполнение материального баланса (расхождение ~10⁻⁹ т), узкие 95%-е
   доверительные интервалы всех откликов и стабилизацию скользящих оценок.
